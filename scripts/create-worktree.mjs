...
function listRemoteBranches(remote) {
	return runOutput("git", ["for-each-ref", `refs/remotes/${remote}`, "--format=%(refname:short)"])
		.split("\n")
		.map((line) => line.trim())
		.filter(Boolean);
}
	...
	try {
		...
		if (remote) {
			...
			if (!cliOptions.remoteBranch && shouldPrompt) {
				remoteBranch =
					(await rl.question(`Remote branch to track/publish (default: ${branchName}): `)).trim() ||
					branchName;
			}
		...
		if (branchExists) {
			...
			run("git", ["worktree", "add", "-b", branchName, worktreePathInput, `${remote}/${remoteBranch}`]);
		} else {
