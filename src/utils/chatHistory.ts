...
export function buildGroupChatRosterSystemPrompt(args: {
	...
	lines.push(
		"Chat transcript format: each message begins with 'SpeakerName: ...'. Do not copy that formatting in your replies."
	);
	lines.push("</system>");
	return "\n" + lines.join("\n");
}
...
export function buildDynamicGroupChatRosterSystemPrompt(args: {
	...
	if (args.allowPings) {
		lines.push(
			"To get a participant's attention, mention them with @<uuid> (NOT @Name). Names may contain spaces."
		);
	}
	lines.push("Participant roster:");
 ...
 */
export function buildSpeakerToneExamplesSystemPrompt(args: { speakerName: string; toneExamples: string[] }): string {
	...
	const lines: string[] = [];
	lines.push(
		`<system>Tone examples for ${speakerName}. Use these as style guidance and stay in character. Do NOT include a speaker prefix like "${speakerName}:" in your reply.</system>`
	);
