...
export function buildPersonalityInstructionMessages(
	...
	const makeModelPart = (text: string) =>
		options?.modelTextThoughtSignature ? { text, thoughtSignature: options.modelTextThoughtSignature } : { text };
	...
	const messages: Content[] = [
		...
		},
		{
			...
			parts: [
				makeModelPart(
					`Very well, from now on, I will be acting as the personality you have chosen. I'm ${personality.name}, and will continue this chat as your desired personality.`
				)
			]
		}
	];
	...
	if (personality.toneExamples && personality.toneExamples.length > 0) {
		messages.push(
			...
			},
			{
				...
				parts: [
					makeModelPart(
						`Understood. I am ready to respond as '${personality.name}'. Please proceed with your questions.`
					)
				]
			}
		);
	...
	(personality.toneExamples || []).slice(0, TONE_QUESTIONS.length).forEach((toneExample, index) => {
