...
export function showMessageDebugModal(debugInfo?: MessageDebugInfo) {
	...
	const requestSlugSummary =
		debugInfo?.requestSlug ||
		(debugInfo?.requestSlugs?.length ? `Multiple (${debugInfo.requestSlugs.length})` : undefined);
	...
	slugValue.textContent =
		requestSlugSummary || (debugInfo?.premiumEndpointEnabled ? "Pending or unavailable" : "Not used");
