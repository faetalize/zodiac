...
async function appendMessageToChat(args: {
	...
	return (
		(await chatsService.mutateChat(chatId, (chat) => {
...
async function appendRequestSlugToMessage(args: {
	chatId: ChatId;
	messageIndex: number;
	requestSlug?: string;
}): Promise<void> {
...
async function loadDynamicContext(args: { chatId: ChatId; shouldEnforceThoughtSignaturesInHistory: boolean }): Promise<{
	chat: DbChat;
	participants: string[];
...
async function respondAsPersona(args: {
	...
	try {
		...
		const systemInstructionText =
			((await settingsService.getSystemPrompt("dynamic")).parts?.[0].text ?? "") +
			args.rosterSystemPrompt +
			tonePrompt;

		const instruction =
			`<system>You are participating in a dynamic group chat.\n` +
		...
		if (args.isPremiumEndpointPreferred) {
			...
			if (isOpenRouterModel(settings.model)) {
				...
				const config: GenerateContentConfig = {
					maxOutputTokens: parseInt(settings.maxTokens),
					temperature: modelSupportsTemperature(settings.model)
						? parseInt(settings.temperature) / 100
						: undefined,
					systemInstruction: { parts: [{ text: systemInstructionText }] } as Content,
					safetySettings: settings.safetySettings,
					responseMimeType: "text/plain",
		...
		if (typeof args.rootUserMessageIndex === "number") {
			await appendRequestSlugToMessage({
				chatId: args.chatId,
				messageIndex: args.rootUserMessageIndex,
				requestSlug
			});
		}
