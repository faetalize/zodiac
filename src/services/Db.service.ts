...
export async function setupDB() {
	...
	db.version(6)
		...
		.upgrade(async (tx) => {
			...
			const oldPersonalities = await tx
				.table("personalities")
				.toArray()
				.catch(() => [] as any[]);
		...
		.upgrade(async (tx) => {
			...
			for (const personality of personalities) {
				await tx.table("personalities").put({
					...
					lastModified:
						typeof personality.lastModified === "number"
							? personality.lastModified
							: typeof personality.dateAdded === "number"
								? personality.dateAdded
								: now
				});
			}
		});
