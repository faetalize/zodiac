 ...
 */
async function encryptBlob(key: CryptoKey, data: Uint8Array): Promise<{ ciphertext: Uint8Array; iv: Uint8Array }> {
	const iv = globalThis.crypto.getRandomValues(new Uint8Array(12));
	const ciphertext = new Uint8Array(
		await globalThis.crypto.subtle.encrypt({ name: "AES-GCM", iv: iv as BufferSource }, key, data as BufferSource)
	);
 ...
 */
async function decryptBlob(key: CryptoKey, ciphertext: Uint8Array, iv: Uint8Array): Promise<Uint8Array> {
 ...
 */
export async function uploadEncryptedBlob(data: Uint8Array, mimeType: string): Promise<BlobReference> {
 ...
 */
export async function downloadDecryptedBlob(ref: BlobReference): Promise<{ data: Uint8Array; mimeType: string }> {
	// Check cache first
