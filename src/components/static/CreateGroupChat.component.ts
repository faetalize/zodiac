...
const searchInput = must(
	document.querySelector<HTMLInputElement>("#group-chat-persona-search"),
	"#group-chat-persona-search"
);
const listContainer = must(
	document.querySelector<HTMLDivElement>("#group-chat-persona-list"),
	"#group-chat-persona-list"
);
const selectedCount = must(
	document.querySelector<HTMLDivElement>("#group-chat-selected-count"),
	"#group-chat-selected-count"
);
const turnOrderContainer = must(
	document.querySelector<HTMLDivElement>("#group-chat-turn-order"),
	"#group-chat-turn-order"
);
...
const dynamicSettingsSection = must(
	document.querySelector<HTMLDivElement>("#group-chat-dynamic-settings"),
	"#group-chat-dynamic-settings"
);
const rpgSettingsSection = must(
...
const guardApplyAll = must(
	...
	"#group-chat-guard-apply-all"
);
const guardApplyAllValue = must(
	document.querySelector<HTMLSpanElement>("#group-chat-guard-apply-all-value"),
	"#group-chat-guard-apply-all-value"
);
const guardApplyAllBtn = must(
	document.querySelector<HTMLButtonElement>("#group-chat-guard-apply-all-btn"),
	"#group-chat-guard-apply-all-btn"
);
...
function renderTurnOrder(): void {
	...
	for (let index = 0; index < turnOrder.length; index++) {
		...
		row.addEventListener("dragend", () => {
			...
			turnOrderContainer
				.querySelectorAll(".group-chat-turn-order-item")
	...
	const items: PersonaListItem[] = [
		{
			...
			image: defaultPersona.image,
			independence: Number((defaultPersona as any)?.independence ?? 0)
		},
		...local.map((p) => ({
			id: p.id,
			name: p.name,
			image: p.image,
			independence: Number((p as any)?.independence ?? 0)
		}))
	];
