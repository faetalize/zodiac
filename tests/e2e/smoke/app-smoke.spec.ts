...
async function readAttachmentInputState(page: Page): Promise<{ names: string[]; previewNames: string[] }> {
    return await page.evaluate(() => {
        ...
        const previewNames = Array.from(document.querySelectorAll<HTMLElement>("#attachment-preview .attachment-container"))
            .map((element) => {
                return element.querySelector<HTMLElement>(".attachment-name")?.textContent
...
async function dispatchAttachmentDrop(page: Page, files: Array<{ name: string; mimeType: string; contents: string }>): Promise<void> {
...
test("user sends a message and sees a mocked reply", async ({ page }) => {
    await stubExternalTraffic(page, [
        { text: "Smoke Test Thread" },
        { text: "Mocked smoke reply." },
    ]);
    await seedLocalSettings(page);
    await page.goto("/");
...
test("user attaches a file by drag and drop", async ({ page }) => {
    ...
    await expect.poll(async () => readAttachmentInputState(page)).toEqual({
        names: ["notes.txt"],
