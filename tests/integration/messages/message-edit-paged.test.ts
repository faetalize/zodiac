...
describe("paged long-chat message editing", () => {
	...
	it("edits absolute message index in a 150 message paged chat", async () => {
		...
		await waitForCondition(() => {
			const rerenderedTarget = document.querySelector<HTMLElement>(
				`.message[data-chat-index='100'] .message-text`
			);
		...
		expect(document.querySelector(`.message[data-chat-index='99']`)).toBeNull();
		expect(
			document.querySelector<HTMLElement>(`.message[data-chat-index='100'] .message-text`)?.textContent
		).toContain("Edited absolute message 100");
		expect(
			document.querySelector<HTMLElement>(`.message[data-chat-index='101'] .message-text`)?.textContent
		).toContain("Long chat message 101");
	...
	it("loads older messages without remapping absolute chat indices", async () => {
		...
		const initiallyVisibleMessages = Array.from(
			document.querySelectorAll<HTMLElement>(".message[data-chat-index]")
		);
		...
		expect(
			document.querySelector<HTMLElement>(`.message[data-chat-index='50'] .message-text`)?.textContent
		).toContain("Long chat message 050");
		expect(
			document.querySelector<HTMLElement>(`.message[data-chat-index='99'] .message-text`)?.textContent
		).toContain("Long chat message 099");
		expect(
			document.querySelector<HTMLElement>(`.message[data-chat-index='100'] .message-text`)?.textContent
		).toContain("Long chat message 100");
		expect(
			document.querySelector<HTMLElement>(`.message[data-chat-index='149'] .message-text`)?.textContent
		).toContain("Long chat message 149");
