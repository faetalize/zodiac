...
describe("Message send lifecycle", () => {
	...
	it("sends a message and stores ordered mocked replies in an RPG group chat", async () => {
		...
		expect(
			visiblePersistedMessages.map((message) => ({
				role: message.role,
		...
		expect(
			visibleDomMessages.map((element) => {
				return (
					element.querySelector<HTMLElement>(".message-text-content")?.textContent ??
					element.querySelector<HTMLElement>(".message-text")?.textContent ??
					undefined
		...
		).toEqual(["Advance the round", "Alpha takes the lead.", "Beta follows up."]);
	});
	...
	it("sends a message and stores a mocked reply in a dynamic group chat", async () => {
		...
		await waitForCondition(
			async () => vi.mocked(openRouterService.requestOpenRouterCompletion).mock.calls.length === 1,
			"Timed out waiting for dynamic provider call"
		);
		...
		await waitForCondition(
			async () => getVisibleMessageElements().length === 2,
			"Timed out waiting for dynamic response to render"
		);
		expect(getVisibleMessageTexts()).toEqual(["Step in here", "Dynamic Alpha replies to the ping."]);
	});
	...
	it("aborts generation and preserves the partial response safely", async () => {
		...
		vi.mocked(openRouterService.requestOpenRouterCompletion).mockImplementationOnce(
			async ({ onText, onThinking }) => {
		...
		await waitForCondition(
			async () => getVisibleMessageElements().length === 2,
			"Timed out waiting for aborted response DOM to settle"
		);
		...
		expect(
			document.querySelector<HTMLElement>(".message:last-of-type .message-text")?.classList.contains("is-loading")
		).toBe(false);
	});
	...
	it("regenerate prunes only the expected tail", async () => {
		...
		const currentChat = await chatsService.getCurrentChat();
		expect(
			(currentChat?.content ?? []).filter((message) => !message.hidden).map((message) => message.parts[0]?.text)
		).toEqual(["First prompt", "First reply", "Second prompt", "Regenerated second reply"]);
		...
		await waitForCondition(
			async () => getVisibleMessageElements().length === 4,
			"Timed out waiting for regenerated DOM to settle"
		);
		expect(getVisibleMessageTexts()).toEqual([
			"First prompt",
			"First reply",
	...
	it("regeneration in an RPG group chat round prunes later round DOM and state consistently", async () => {
		...
		await waitForCondition(
			async () => getVisibleMessageTexts().length === 3,
			"Timed out waiting for initial RPG round render"
		);
		expect(getVisibleMessageTexts()).toEqual(["Round one prompt", "Beta speaks.", "Delta closes the round."]);
		...
		const roundOneBetaMessage = Array.from(
			document.querySelectorAll<HTMLElement>(".message[data-round-index='1']")
		...
		await waitForCondition(
			async () => vi.mocked(openRouterService.requestOpenRouterCompletion).mock.calls.length === 3,
			"Timed out waiting for RPG regenerate provider calls"
		);
		await chatsService.waitForPendingWrites(chatId);
		...
		expect(
			persistedVisibleMessages.map((message) => ({
				text: message.parts[0]?.text,
				roundIndex: message.roundIndex,
				personalityid: message.personalityid
			}))
		...
		expect(
			(persistedChat?.content ?? []).some(
				(message) =>
					message.hidden &&
					message.personalityid === "persona-rpg-a" &&
					message.parts[0]?.text === "__ai_skip_turn__" &&
		...
		expect(
			(persistedChat?.content ?? []).some(
				(message) =>
					message.hidden &&
					message.personalityid === "persona-rpg-c" &&
					message.parts[0]?.text === "__ai_skip_turn__" &&
					message.roundIndex === 1
			)
		).toBe(false);
		...
		await waitForCondition(
			async () => getVisibleMessageElements().length >= 4,
			"Timed out waiting for group regeneration DOM to settle"
		);
		expect(getVisibleMessageTexts()).toEqual([
			"Round one prompt",
			"Beta regenerated.",
		...
		expect(
			Array.from(roundOneBlock?.querySelectorAll<HTMLElement>(".message") ?? []).map((element) => {
				return (
					element.querySelector<HTMLElement>(".message-text-content")?.textContent ??
					element.querySelector<HTMLElement>(".message-text")?.textContent ??
					undefined
		...
		).toEqual(["Round one prompt", "Beta regenerated.", "Gamma regenerated.", "Delta regenerated."]);
